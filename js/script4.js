const text = {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // 1. Déterminer la langue initiale (localStorage a priorité)
        const initialLang = localStorage.getItem('lang') || 'fr';
        const langSelect = document.getElementById('langSelect');
        
        // 2. Appliquer la langue initiale au contenu et au sélecteur
        if (langSelect) {
            applyText(initialLang);
            langSelect.value = initialLang;
        } else {
            // Fallback si le sélecteur n'est pas trouvé
            applyText('fr'); 
        }

        // 3. Écouteur pour les changements de langue
        if (langSelect) {
            langSelect.addEventListener('change', (e) => {
              const newLang = e.target.value;
              localStorage.setItem('lang', newLang); // Sauvegarde dans localStorage
              applyText(newLang); // Application des traductions
            });
        }
    });

    // Rendre toggleTheme globalement accessible car il est appelé via onclick
